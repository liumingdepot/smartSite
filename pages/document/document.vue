	...
	<view class="document">
		...
		<view class="item">
			...
			<uni-swipe-action v-if="show3">
				<uni-swipe-action-item v-for="(item,index) in mycreate" :key="index" :auto-close="false" :show="item.show"
				 :options="shareOptions" @click="mycreateClick($event,index,item)" @change="mycreateChange($event,index)">
	...
	export default {
		...
		methods: {
			...
			collectClick(event, index, item) {
				...
				if (event.index == 1) {
					//查看
					this.$refs.lFile.download(item.url).then(path => {
			...
			collectOnClick(show, index) {
				for (let i = 0; i < this.collectList.length; i++) {
					if (i == index) {
						this.$set(this.collectList, i, { ...this.collectList[i],
							show: !show
						})
					} else {
						this.$set(this.collectList, i, { ...this.collectList[i],
			...
			shareOnClick(show, index) {
				for (let i = 0; i < this.shareList.length; i++) {
					if (i == index) {
						this.$set(this.shareList, i, { ...this.shareList[i],
							show: !show
						})
					} else {
						this.$set(this.shareList, i, { ...this.shareList[i],
			...
			mycreateOnClick(show, index) {
				for (let i = 0; i < this.mycreate.length; i++) {
					if (i == index) {
						this.$set(this.mycreate, i, { ...this.mycreate[i],
							show: !show
						})
					} else {
						this.$set(this.mycreate, i, { ...this.mycreate[i],
